...
class LetterDictionaries {
    ...
    val preposition_exceptions: Set<String> = setOf("beter", "geven", "ver", "beven", "bezem", "gele")
    ...
    fun remove_accent(letter: String): String {
        val result: String
        when (letter) {
            in setOf("á", "ä") -> result = "a"
            in setOf("é", "ë") -> result = "e"
            in setOf("í", "ï") -> result = "i"
            in setOf("ó",  "ö") -> result = "o"
            in setOf("ú", "ü") -> result = "u"
            else -> {
                throw IllegalArgumentException("Cannot remove accent if letter is not a vowel")
            }
        }
        return result
    }
